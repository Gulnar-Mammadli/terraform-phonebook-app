resource "aws_autoscaling_group" "autoscaling-group-backend" {
  ...
  vpc_zone_identifier = [aws_subnet.first-private-subnet.id, aws_subnet.second-private-subnet.id]
  desired_capacity    = 2
  max_size            = 10
  min_size            = 2
...
resource "aws_autoscaling_group" "autoscaling-group-frontend" {
  ...
  vpc_zone_identifier = [aws_subnet.first-public-subnet.id, aws_subnet.second-public-subnet.id]
  desired_capacity    = 2
  max_size            = 10
  min_size            = 2
